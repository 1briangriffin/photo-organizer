...
def _migration_10_face_cannot_links(conn: sqlite3.Connection) -> None:
    """Durable negative identity decisions. Rejecting a face proposal
    snapshots the detection ids on each side at rejection time, because
    cluster ids/keys are regenerated by every re-clustering run — the
    detections are the only stable anchor. A merge rejection fills
    detections_a/detections_b; an assignment rejection fills detections_a
    plus person_id. Linker/refine/accept consume these as cannot-link
    constraints so a rejected suggestion never resurfaces."""
    conn.execute("""
    CREATE TABLE IF NOT EXISTS face_cannot_links (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        detections_a TEXT NOT NULL,
        detections_b TEXT,
        person_id INTEGER REFERENCES face_persons(id) ON DELETE CASCADE,
        source_action_id INTEGER REFERENCES run_actions(id),
        status TEXT NOT NULL DEFAULT 'active',
        created_by_run_id INTEGER REFERENCES command_runs(id),
        created_at TEXT NOT NULL,
        note TEXT,
        CHECK (detections_b IS NOT NULL OR person_id IS NOT NULL)
    );
    """)
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_face_cannot_links_status "
        "ON face_cannot_links(status);"
    )
...
MIGRATIONS = {
    ...
    10: _migration_10_face_cannot_links,
}
