...
def test_get_merges_built_on_evicted_evidence_excludes_dead_clusters(db):
    """Regression: cluster ids are regenerated by every re-clustering run,
    so an applied merge from an old generation routinely references a
    since-superseded id on one or both sides. On the real catalog an
    earlier version of this diagnostic returned 535 raw hits, but 523 of
    them (98%) pointed at a cluster that no longer exists live -- nothing
    unlink-cluster or Cluster Review can act on. Only merges where BOTH
    sides are still 'proposed'/'accepted' right now should be returned."""
    db_path, conn, face_ops = db
    run_id = _start_run(conn, command="cluster")
    ...
    real_a = _add_detection(face_ops, run_id=run_id, file_id=2, embedding=ALICE)
    real_b = _add_detection(face_ops, run_id=run_id, file_id=3, embedding=ALICE2)
    cluster_a = _make_cluster(conn, face_ops, run_id=run_id, key="dead-a",
                              era_start="2010-01-01T00:00:00",
                              era_end="2010-01-01T00:00:00",
                              representative=ALICE,
                              detection_ids=[intruder, real_a])
    cluster_b = _make_cluster(conn, face_ops, run_id=run_id, key="dead-b",
                              era_start="2015-01-01T00:00:00",
                              era_end="2015-01-01T00:00:00",
                              representative=ALICE2,
                              detection_ids=[real_b])
    ...
    action_id = _insert_applied_merge(
        conn, run_id, method="same_event_tracklet", key="merge-dead-a-b",
        confidence=70, cluster_a=cluster_a, cluster_b=cluster_b,
        applied_at="2020-06-01T00:00:00Z",
        signals={"tracklet_pairs": 1, "same_photo_overlap": 0},
    )
    ...
    # Sanity check: with both sides still live, this must be flagged.
    assert action_id in {
        c["action_id"] for c in face_ops.get_merges_built_on_evicted_evidence()
    }

    # A subsequent re-cluster supersedes cluster_a (its id no longer live).
    recluster_run = _start_run(conn, command="cluster")
    conn.execute(
        "UPDATE face_clusters SET status='superseded', updated_by_run_id=? "
        "WHERE id=?", (recluster_run, cluster_a),
    )
    conn.commit()

    candidates = face_ops.get_merges_built_on_evicted_evidence()
    assert action_id not in {c["action_id"] for c in candidates}, (
        "a merge referencing a now-superseded cluster id isn't actionable "
        "via unlink-cluster and must not be reported"
    )
